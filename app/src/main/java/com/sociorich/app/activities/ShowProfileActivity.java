...
//                            String wrkPlace = response.getString("workplace");
                            ...
                            try{
                                ...
                                for(int i=0;i<interestCategories.length();i++){
                                    ...
                                    allIntrest = allIntrest+"#"+catName+"  ";
                                }
                                if(allIntrest.length()>50){
                                    ConstantMethods.twoColoredText(intrestAllTxt,allIntrest,"\nShow more");
                                    String finalAllIntrest = allIntrest;
                                    intrestAllTxt.setOnClickListener(v-> {
                                        ConstantMethods.showAllInterest(finalAllIntrest, ShowProfileActivity.this);
                                    });
                                }
                                else {
                                    intrestAllTxt.setText(allIntrest);
                                }
                            }catch (Exception e){
                                e.getStackTrace();
                            }
...
//
//                                    .into(profileImg);
                            Glide.with(ShowProfileActivity.this).load(profileUrl).into(profileImg);
